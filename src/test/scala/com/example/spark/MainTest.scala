...
class MainTest {
  ...
  def createsWorkingSparkSession(): Unit = {
    val spark = Main.newSparkSession("unit-test")

    try {
      assertEquals(3L, spark.range(3).count())
    } finally {
      spark.stop()
    }
  }
  ...
  def createsPartitionedIcebergTableWithUpdateFriendlyProperties(): Unit = {
    ...
    try {
      ...
      Main.persistToIceberg(input)

      val createStatement = spark.sql("SHOW CREATE TABLE local.production.features").collect().map(_.getString(0)).mkString("\n")
  ...
  def mergesRowsAndEvolvesSchemaWithoutNullingMissingColumns(): Unit = {
    ...
    try {
      ...
      Main.persistToIceberg(initialBatch)
      ...
      Main.persistToIceberg(secondBatch)
  ...
  def mainLoadsParquetAndPersistsToIceberg(): Unit = {
    ...
    val writerSpark = Main.newSparkSession("main-parquet-writer")
    ...
    try {
      ...
      Main.main(Array(parquetPath.toString))
  ...
  private def newIcebergSparkSession(warehouse: Path = Files.createTempDirectory("iceberg-warehouse")): org.apache.spark.sql.SparkSession = {
    Main.newSparkSession(
      ...
      Map(
        ...
        "spark.sql.catalog.local.warehouse" -> warehouse.toUri.toString
      )
    )
  }
}
