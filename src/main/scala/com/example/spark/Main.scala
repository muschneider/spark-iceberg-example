 ...
 * CLI entrypoint.
 *
 * Loads a Parquet dataset and upserts it into the configured Iceberg table.
 * The same code path runs locally (tests, `java -jar`) and on a managed cluster
 * like Google Cloud Dataproc; cluster vs. local is auto-detected by
...
object Main {
  ...
  def persistToIceberg(
    ...
    IcebergWriter.upsert(dataFrame, catalog, tableName)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark =
      if (SparkSessions.isClusterRun) SparkSessions.cluster(AppName)
      else SparkSessions.localWithLocalIcebergCatalog(AppName)
